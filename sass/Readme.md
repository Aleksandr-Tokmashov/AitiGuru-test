...
folder are mapped to the application's root namespace, 'AitiGuru.test'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "AitiGuru.test"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'AitiGuru.test' in the same way
as `"AitiGuru.test/sass/src"`.
